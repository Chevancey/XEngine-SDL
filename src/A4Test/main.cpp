...
int main()
{
	...
	//SoundDevice::Instance().SetGain(0.1f);
	//SoundDevice::get()->SetAttunation(AL_EXPONENT_DISTANCE_CLAMPED);

	//uint32_t sound1 = SoundEffect::get()->addSoundEffect("assets/siren.wav");
	//SoundSource soundScource;
	//soundScource.Play(sound1);
	//soundScource.SetLooping(true);
	...
	while (isOpen)
	{
		...
		//SoundDevice::get()->Update();
		//SoundDevice::get()->setPosition(camera.GetPosition());
		//SoundDevice::get()->Update();
