...
{
	BoxShape(float width, float height, float radius = 0.f, bool trigger = false);
	BoxShape(float x, float y, float width, float height, float radius = 0.f, bool trigger = false);
...
{
	CircleShape(float radius, bool trigger = false);
...
{
	ConvexShape(std::vector<Vector2f> pos, float radius = 0.f, bool trigger = false);
	ConvexShape(const Model& model, Matrix3f transformation = Matrix3f::Identity(), float radius = 0.f, bool trigger = false);
...
{
	SegmentShape(const Vector2f& from, const Vector2f& to, float radius = 0.f, bool trigger = false);
